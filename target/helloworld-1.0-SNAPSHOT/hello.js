...
function enableButtons () {
	...
	// Update the button label now that the button is active
	btn.value="Click me for a personal greeting by time of the day";
	
	
    btn.value="Click me for a personal greeting";
	
	// Set the onclick action for the second button
	btn = document.getElementById("input_greet_with_message");
	btn.onclick=function(){greetWithMessage();};
	
	// Update the button label now that the button is active
	btn.value="Click me for a personal greeting with message";
}
...
function greetByPeriod(){
	...
	var period = document.getElementById("name_field").value;
	
	var request = gapi.client.helloworldendpoints.greetByPeriod({'name': name, 'period':period});
	request.execute(sayHelloCallback);
}

function greetWithMessage(){
	
	var name = document.getElementById("name_field").value;
	
	var period = document.getElementById("name_field").value;
	
	var gender = document.getElementById("name_field").value;
	
	var request = gapi.client.helloworldendpoints.greetByPeriod({'name': name, 'period':period, 'gender':gender});
	request.execute(sayHelloCallback);
}
